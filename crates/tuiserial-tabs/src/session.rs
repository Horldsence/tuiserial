...
use tuiserial_core::{
    ...
    types::{AppendMode, DisplayMode, FlowControl, FocusedField, Parity, TxMode},
};
...
impl SerialSession {
    ...
    /// Toggle TX mode, converting existing input when switching
    pub fn toggle_tx_mode(&mut self) {
        self.tx_input = tuiserial_core::convert_tx_input(&self.tx_input, self.tx_mode);
    ...
    pub fn next_baud_rate(&mut self, baud_rate_options: &[u32]) -> bool {
        if !self.can_modify_config() {
            return false;
        }
        if let Some(selected) = self.baud_rate_state.selected() {
            let next = (selected + 1) % baud_rate_options.len();
            self.baud_rate_state.select(Some(next));
            self.config.baud_rate = baud_rate_options[next];
            true
        } else {
            false
        }
    ...
    pub fn prev_baud_rate(&mut self, baud_rate_options: &[u32]) -> bool {
        if !self.can_modify_config() {
            return false;
        }
        if let Some(selected) = self.baud_rate_state.selected() {
            let next = if selected == 0 {
                baud_rate_options.len() - 1
            } else {
                selected - 1
            };
            self.baud_rate_state.select(Some(next));
    ...
    pub fn toggle_parity(&mut self, parity_options: &[Parity]) -> bool {
        if !self.can_modify_config() {
            return false;
        }
        if let Some(selected) = self.parity_state.selected() {
            let next = (selected + 1) % parity_options.len();
            self.parity_state.select(Some(next));
            self.config.parity = parity_options[next];
            true
        } else {
            false
        }
    ...
    pub fn toggle_flow_control(&mut self, flow_control_options: &[FlowControl]) -> bool {
        if !self.can_modify_config() {
            return false;
        }
        if let Some(selected) = self.flow_control_state.selected() {
            let next = (selected + 1) % flow_control_options.len();
            self.flow_control_state.select(Some(next));
            self.config.flow_control = flow_control_options[next];
            true
        } else {
            false
        }
    ...
    pub fn next_data_bits(&mut self, data_bits_options: &[u8]) -> bool {
        if !self.can_modify_config() {
            return false;
        }
        if let Some(selected) = self.data_bits_state.selected() {
            let next = (selected + 1) % data_bits_options.len();
            self.data_bits_state.select(Some(next));
            self.config.data_bits = data_bits_options[next];
            true
        } else {
            false
        }
    ...
    pub fn next_stop_bits(&mut self, stop_bits_options: &[u8]) -> bool {
        if !self.can_modify_config() {
            return false;
        }
        if let Some(selected) = self.stop_bits_state.selected() {
            let next = (selected + 1) % stop_bits_options.len();
            self.stop_bits_state.select(Some(next));
            self.config.stop_bits = stop_bits_options[next];
            true
        } else {
            false
        }
    ...
    pub fn select_port(&mut self, index: usize, ports: &[String]) -> bool {
        if !self.can_modify_config() {
            return false;
        }
        if index < ports.len() {
            self.port_list_state.select(Some(index));
            self.config.port = ports[index].clone();
            true
        } else {
            false
        }
    ...
    pub fn next_append_mode(&mut self, append_mode_options: &[AppendMode]) {
        if let Some(selected) = self.append_mode_state.selected() {
            let next = (selected + 1) % append_mode_options.len();
            self.append_mode_state.select(Some(next));
            self.tx_append_mode = append_mode_options[next];
        }
    }

    /// Cycle to previous append mode
    pub fn prev_append_mode(&mut self, append_mode_options: &[AppendMode]) {
        if let Some(selected) = self.append_mode_state.selected() {
            let next = if selected == 0 {
                append_mode_options.len() - 1
            } else {
                selected - 1
            };
            self.append_mode_state.select(Some(next));
...
impl SessionManager {
    ...
    /// Update all sessions' notifications
    pub fn update_all_notifications(&mut self) {
        for session in &mut self.sessions {
            session.update_notifications();
        }
    }
}
